...
def main():
    ...
    try:
        ...
        df_gps_filtrado,gps_pos, gps_final = preprocessor.compute_positions(df_inter, preprocessor.config)
        gps_time = pd.to_numeric(df_gps_filtrado['time'], errors='coerce').to_numpy()
        gps_pos_interp = np.zeros((len(time), 2))

        for dim in range(2):
            interp_func = interp1d(gps_time, gps_pos[:, dim], bounds_error=False, fill_value="extrapolate")
            gps_pos_interp[:, dim] = interp_func(time)
        n = len(gps_pos)
        mid = n // 2
        ...
        gps_lat = df_gps_filtrado['lat'] if not df_gps_filtrado.empty else np.full(len(pos_kalman), np.nan)
        gps_lng = df_gps_filtrado['lng'] if not df_gps_filtrado.empty else np.full(len(pos_kalman), np.nan)
